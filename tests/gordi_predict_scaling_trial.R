...
gordi_read(m, env, scaling = 'sites') |> 
  gordi_species() |> 
  gordi_predict(scaling_coefficient = 1)


o <- gordi_read(m, scaling = 'species')




p <- ggplot() + 
  geom_segment(data = o$species_scores,
               aes(0, 0, xend = CAP1, yend = MDS1), arrow = arrow(length = unit(0.3, 'cm'))) 
...
predictor_range <- c(xmin_pred = min(o$predictor_scores$CAP1),
                     xmax_pred = max(o$predictor_scores$CAP1),
                     ymin_pred = min(o$predictor_scores$MDS1),
                     ymax_pred = max(o$predictor_scores$MDS1))



(max(abs(plot_range)) / max(abs(predictor_range))) * 0.9


p + geom_segment(data = o$predictor_scores,
                 aes(0, 0,
                     xend = CAP1 * (max(abs(plot_range)) / max(abs(predictor_range))) * 0.8,
                     yend = MDS1 * (max(abs(plot_range)) / max(abs(predictor_range))) * 0.8),
